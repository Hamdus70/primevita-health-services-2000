  ...
  return (
    <div id="custom-language-container" className="relative inline-block" ref={containerRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 bg-[#10837f]/5 hover:bg-[#10837f]/10 active:scale-95 border border-[#10837f]/20 hover:border-[#10837f]/40 transition-all duration-200 px-3.5 py-1.5 rounded-full shadow-sm text-[#10837f] font-bold text-xs select-none cursor-pointer h-9 shrink-0 outline-none"
        aria-label="Select Language"
        title="Translate Website"
      >
        {/* Authentic Multi-Color Google Icon Logo */}
        <svg id="google-logo-svg" className="w-4 h-4 shrink-0 transition-transform duration-300 hover:rotate-12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span id="active-flag-span" className="text-sm">{activeLanguage.flag}</span>
        <span id="active-lang-name" className="uppercase tracking-wider hidden xs:inline-block">{activeLanguage.name}</span>
        <ChevronDown id="active-chevron-down" className={`w-3.5 h-3.5 opacity-70 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
