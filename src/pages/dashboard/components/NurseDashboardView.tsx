...
import { MapPin, Clock, Stethoscope, Table2, FileText, Pill, Users, ArrowLeft, Activity, Droplets, User, Calendar, Plus, History, Camera, CheckCircle2, AlertCircle, ClipboardList, MessageSquare } from 'lucide-react';
...
export function NurseDashboardView({ role = 'nurse' }: { role?: 'nurse' | 'caregiver' }) {
    ...
    const fetchProfile = async () => {
      ...
      if (profSnap.exists()) {
        ...
        setProfile({
            ...
            email: user.email,
            staffId: `HSP-NUR-${user.id.substring(0, 4).toUpperCase()}`,
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading Nurse Dashboard...</div>;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-[#0e4e5e] mb-2">
                {editMode ? 'Update Clinical Profile' : 'Complete Your Clinical Profile'}
            </h2>
            <p className="text-gray-600">
                {editMode ? 'Update your biodata details.' : 'Per hospital policy, all nursing staff must verify their biodata before accessing patient records.'}
            </p>
        </div>
  ...
  if (selectedPatient) {
    return <PatientFileView patient={selectedPatient} onBack={() => setSelectedPatientId(null)} nurse={profile} role={role} user={user} />;
  }
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
             <div className="flex items-center gap-6">
                 ...
                 <div className="w-20 h-20 rounded-full border-2 border-gray-100 overflow-hidden shrink-0">
                     {profile.photoUrl ? (
                         <img src={profile.photoUrl} alt="Nurse Profile" className="w-full h-full object-cover" />
                     ) : (
...
function PatientFileView({ patient, onBack, nurse, role, user }: { patient: any, onBack: () => void, nurse: any, role: string, user: any }) {
    ...
    useEffect(() => {
       ...
       const vUnsub = onSnapshot(query(collection(db, 'vitalSigns'), where('patientId', '==', patient.id)), (snap) => {
        ...
        const nUnsub = onSnapshot(query(collection(db, 'clinicalNotes'), where('patientId', '==', patient.id)), (snap) => {
            const v: any[] = [];
            const io: any[] = [];
            snap.forEach(d => {
                const dta = d.data();
                if (dta.noteType === 'intake_output' && dta.intakeOutputDetails) {
                    ...
                    v.push({
                        ...dta,
                        time: dta.createdAt?.toDate?.()?.toLocaleString() || new Date().toLocaleString(),
                        text: dta.plan || dta.subjective || dta.objective || ''
                    });
                }
            });
            v.sort((a,b) => new Date(b.time).getTime() - new Date(a.time).getTime());
       ...
       const cpUnsub = onSnapshot(query(collection(db, 'carePlans'), where('patientId', '==', patient.id)), (snap) => {
           const v: any[] = [];
           snap.forEach(d => {
       ...
       return () => { vUnsub(); nUnsub(); cpUnsub(); rxUnsub(); };
    }, [patient.id]);
    ...
    // Careplan temporary fields
    const [cpDiagnosis, setCpDiagnosis] = useState('Impaired physical mobility related to neuromuscular impairment as evidenced by inability to move purposefully.');
    const [cpObjective, setCpObjective] = useState('Patient will maintain existing range of motion and show increased strength within 2 weeks.');
    const [cpIntervention, setCpIntervention] = useState('1. Assist with passive ROM exercises QD.\n2. Encourage use of ambulatory aids.\n3. Ensure safe environment to prevent falls.');
    ...
    const saveNote = async () => {
       try {
           ...
           toast.success("Nursing report saved successfully!");
           setNoteText('');
       } catch (err: any) {
    ...
    const saveCarePlan = async () => {
        try {
           await addDoc(collection(db, 'carePlans'), {
               ...
               intervention: cpIntervention,
               evaluation: cpEvaluation,
               status: 'active',
               createdAt: serverTimestamp()
           });
           ...
           toast.error("Failed to save care plan: " + err.message);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <Card className="border-0 shadow-sm ring-1 ring-[#10837f]/30 bg-gradient-to-r from-[#f8fcfc] to-white relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 border-b border-l bg-white rounded-bl-xl border-gray-100 shadow-sm text-xs text-gray-500 flex items-center gap-2">
                    <User className="w-3 h-3" /> Assigned Nurse: <strong className="text-[#10837f]">{nurse.fullName}</strong>
                </div>
            ...
            {/* B. Clinical Modules (Tabbed Layout) */}
            <Tabs defaultValue="vitals" className="w-full">
                <TabsList className="grid grid-cols-2 md:grid-cols-6 h-auto md:h-14 bg-white border shadow-sm rounded-xl p-1 gap-1 mb-6 text-sm font-medium">
                    ...
                    {role !== 'caregiver' && <TabsTrigger value="careplan" className="rounded-lg data-[state=active]:bg-[#10837f] data-[state=active]:text-white">Care Plan</TabsTrigger>}
                    ...
                    <TabsTrigger value="notes" className="rounded-lg data-[state=active]:bg-[#10837f] data-[state=active]:text-white">Nursing Report</TabsTrigger>
                ...
                {/* 2. Care Plan Module */}
                {role !== 'caregiver' && <TabsContent value="careplan">
                    <Card className="border-0 shadow-sm ring-1 ring-gray-100">
                        <CardHeader className="bg-gray-50/50 border-b pb-4">
                            <CardTitle className="text-lg flex items-center gap-2"><ClipboardList className="w-5 h-5 text-[#10837f]" /> Nursing Care Plan</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6">
                            <div className="overflow-x-auto pb-4">
                                <table className="w-full border-collapse min-w-[800px]">
                                    <thead>
                                        <tr className="bg-[#f8fcfc] text-[#0e4e5e] text-sm border-y border-gray-200">
                                            <th className="p-3 font-semibold text-left w-1/4">Nursing Diagnosis</th>
                                            <th className="p-3 font-semibold text-left w-1/4">Goals / Outcomes</th>
                                            <th className="p-3 font-semibold text-left w-1/4">Interventions</th>
                                            <th className="p-3 font-semibold text-left w-1/4">Evaluation</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y relative">
                                        {/* Existing Care Plans */}
                                        {carePlans.map(cp => (
                                            <tr key={cp.id} className="bg-white group transition-colors hover:bg-gray-50/50 border-b">
                                                <td className="p-3 align-top whitespace-pre-wrap">{cp.diagnosis}</td>
                                                <td className="p-3 align-top whitespace-pre-wrap">{cp.objective}</td>
                                                <td className="p-3 align-top whitespace-pre-wrap">{cp.intervention}</td>
                                                <td className="p-3 align-top whitespace-pre-wrap">{cp.evaluation}</td>
                                            </tr>
                                        ))}

                                        {/* New Entry Row */}
                                        <tr className="bg-white group transition-colors hover:bg-gray-50/50">
                                            <td className="p-3 align-top">
                                                <Textarea value={cpDiagnosis} onChange={e => setCpDiagnosis(e.target.value)} className="min-h-[140px] bg-transparent border-gray-200 focus:bg-white resize-none text-sm leading-relaxed" />
                                            </td>
                                            <td className="p-3 align-top">
                                                <Textarea value={cpObjective} onChange={e => setCpObjective(e.target.value)} className="min-h-[140px] bg-transparent border-gray-200 focus:bg-white resize-none text-sm leading-relaxed" />
                                            </td>
                                            <td className="p-3 align-top">
                                                <Textarea value={cpIntervention} onChange={e => setCpIntervention(e.target.value)} className="min-h-[140px] bg-transparent border-gray-200 focus:bg-white resize-none text-sm leading-relaxed" />
                                            </td>
                                            <td className="p-3 align-top">
                                                <Textarea value={cpEvaluation} onChange={e => setCpEvaluation(e.target.value)} placeholder="Evaluate progress here..." className="min-h-[140px] bg-transparent border-gray-200 focus:bg-white resize-none text-sm leading-relaxed" />
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <div className="flex justify-end gap-3 mt-4">
                                 <Button className="bg-[#10837f] hover:bg-[#0c6b68]" onClick={saveCarePlan}>Save Care Plan</Button>
                            </div>
                        </CardContent>
                    </Card>
                </TabsContent>}
                ...
                <TabsContent value="notes">
                   <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <CardContent className="pt-6">
                                <div className="space-y-4">
                                    <AIInvestigatorTextarea 
                                        className="min-h-[300px] p-4 text-sm leading-relaxed border-gray-200 focus-visible:ring-[#10837f] shadow-inner bg-gray-50/50" 
                                        ...
                                        staffName={nurse?.fullName || nurse?.firstName || "Nurse"}
                                        role="Nurse"
                                    />
                       ...
                       <Card className="border-0 shadow-sm ring-1 ring-gray-100 max-h-[600px] flex flex-col">
                            <CardHeader className="bg-gray-50/50 border-b pb-4 shrink-0">
                                <CardTitle className="text-lg flex items-center gap-2"><History className="w-5 h-5 text-gray-500" /> Nursing Record History</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-4 overflow-y-auto">
                                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gray-200 before:to-transparent">
                                    {notesLog.map((log: any, i) => (
                                        <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active text-left">
                                            ...
                                            <div className="w-[calc(100%-2.5rem)] md:w-[calc(50%-1.5rem)] ml-10 md:ml-0 p-4 rounded-xl border border-gray-100 bg-white shadow-sm">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="font-bold text-[#0e4e5e] text-sm">
                                                        {log.authorId === 'Voice AI Triage Nurse' ? '🤖 Telehealth Voice AI Triage' : (log.authorName || nurse.fullName)}
                                                    </span>
                                                    <time className="text-xs font-medium text-gray-500 bg-gray-50 px-2 py-1 rounded">{log.time}</time>
                                                </div>
                                                {log.noteType === 'telehealth_call' ? (
                                                    <div className="space-y-3 mt-2 border-l-2 border-teal-500 pl-3">
                                                        <div className="text-xs font-semibold text-teal-800 bg-teal-50 px-2 py-1 rounded inline-block">📞 Voice Hotline Call Session</div>
                                                        <div className="text-xs text-gray-700 bg-slate-50 p-2.5 rounded border">
                                                            <strong className="text-[#0e4e5e]">Subjective (Chief Complaint & History)</strong>
                                                            <p className="mt-1 leading-relaxed text-gray-650">{log.subjective}</p>
                                                        </div>
                                                        {log.objective && (
                                                            <div className="text-xs text-gray-700 bg-slate-50 p-2.5 rounded border">
                                                                <strong className="text-red-700">Objective (Red Flags Screening)</strong>
                                                                <p className="mt-1 leading-relaxed text-gray-650">{log.objective}</p>
                                                            </div>
                                                        )}
                                                        {log.assessment && (
                                                            <div className="text-xs text-gray-700 bg-slate-50 p-2.5 rounded border">
                                                                <strong className="text-amber-800">Assessment (AI Diagnostic Reasoning)</strong>
                                                                <p className="mt-1 leading-relaxed text-gray-650">{log.assessment}</p>
                                                            </div>
                                                        )}
                                                        {log.plan && (
                                                            <div className="text-xs text-gray-700 bg-slate-50 p-2.5 rounded border">
                                                                <strong className="text-emerald-800">Plan (Physician Action & Care Plan)</strong>
                                                                <p className="mt-1 leading-relaxed text-gray-650">{log.plan}</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                ) : (
